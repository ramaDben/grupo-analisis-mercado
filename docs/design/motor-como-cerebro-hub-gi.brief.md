# El motor como cerebro de un hub inteligente — Brief para Grupo Inteligencia
...
> **Estado de la decisión:** explorar viabilidad y alcance, no aprobar un desarrollo.
...
> **La demostración concreta** (recomendada para acompañar este brief): tomar la salida real de un día y mostrarla lado a lado — la misma inteligencia renderizada como la vería un ejecutivo y como la vería un cliente. Eso convierte "podría servir" en "miren, ya sirve".
...
No pedir aprobar un desarrollo completo. Pedir dos cosas acotadas que despejan el camino:
...
## 10. Anexo interno — notas francas (ELIMINAR en la versión para GI)

> Esta sección es para uso interno antes de exponer el documento. Recortar al exportar a GI.

- **El motor corre hoy local**, conectado a un terminal MT5 con credenciales de cuenta cliente (`MT5_LOGIN`/`MT5_PASSWORD`/`MT5_SERVER`). Para servir un hub 24·7 hay que llevarlo a la nube siempre encendido — ese costo es real y aún no está definido; es el hueco que vuelve "incierto el camino".
- **La Manager API es un bloqueante de verdad**, no un detalle: sin ella, toda la Fase 2 (métricas de cuenta) no existe. Conviene confirmar acceso/costo **antes** de prometérsela a GI.
- **El PRD `broker-cfd-hub-prd.html` lo armé con Perplexity** sin contexto del motor; tratarlo como borrador, no como requerimiento. Lo válido de él es la "regla de oro" (WordPress = experiencia) y la lista de roles; lo demás es genérico.
- **No soy el arquitecto.** Este documento comunica desde mi rol (unir los mundos y probar la idea). La construcción real necesita a alguien técnico; este brief sirve para decidir si vale la pena llamarlo.
- **Postura ante GI:** su primer documento se apoyaba fuerte en WordPress. No pelear contra eso — usar WordPress como capa de experiencia y poner el motor detrás. Es más vendible y respeta su inversión.
